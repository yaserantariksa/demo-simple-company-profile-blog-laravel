<x-admin.layout.admin title="Add New Article" page="Add New Article" home="Articles" :homeurl="route('articles.index')">
    {{-- begin:Add New Artiicle Form --}}
    <div class="card">
        <div class="card-body">
            <form action="{{ route('articles.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <img src="{{ asset('/placeholder.png') }}" alt="placeholder"
                        class="my-3 img-thumbnail rounded mx-auto d-block" style="width: 300px;" id="featured-image">
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="content" id="content" class="form-control @error('content') is-invalid @enderror" hidden>{{ old('content') }}</textarea>
                    <x-admin.editor />
                    @error('content')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <select class="form-select mb-3" name="tags[]" id="tags" multiple aria-label="tags select">
                    ...
                    @foreach ($tags as $t)
                        <option value="{{ $t->id }}"
                            @if (old('tags') != null) @foreach (old('tags') as $otid)
                                            @if ($otid == $t->id)
                                                {{ 'selected' }} @endif
                            @endforeach
                    @endif
                ...
                <div class="form-check mb-3">
                    <input class="form-check-input" type="checkbox" name="is_draft" id="is_draft"
                        onchange="isDraftChecked()" {{ old('is_draft') == null ? '' : 'checked' }}>
